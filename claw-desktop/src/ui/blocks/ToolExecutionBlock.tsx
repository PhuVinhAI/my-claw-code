  ...
  return (
    <div className="my-2 bg-muted/10 rounded-lg border border-border/30 overflow-hidden">
      ...
      <div className="group flex items-center gap-3 px-4 py-3 bg-muted/20 border-b border-border/30">
        ...
        <span className={cn('font-semibold text-sm leading-4 text-foreground/90', isError && 'text-red-400')}>
          {toolName}
        </span>
        {displayLabel && (
          <>
            <span className="text-muted-foreground/30 leading-4">|</span>
            <span className="text-muted-foreground/60 text-xs leading-4">{displayLabel}:</span>
          </>
        )}
        {displayValue && (
          <span className="font-mono truncate flex-1 text-muted-foreground/70 text-xs leading-4">
        ...
        {inlineSummary && (
          <>
            <span className="text-muted-foreground/30 leading-4">|</span>
            <span className={cn(
              "font-mono text-xs leading-4 truncate max-w-md",
        ...
        {isCancelled && (
          <span className="text-red-400 text-xs leading-4 font-medium bg-red-400/10 px-2 py-1 rounded-md border border-red-400/20">
          ...
          {isExpanded && (
            <div className="px-4 py-3 bg-muted/5">
              ...
              {toolName === 'ToolSearch' && 'matches' in parsedOutput && (
                <div className="space-y-3">
                  ...
                  {parsedOutput.matches.length > 0 ? (
                    <div className="flex flex-wrap gap-2">
                      {parsedOutput.matches.map((match: string, idx: number) => (
                        <span
                          key={idx}
                          className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-muted/20 rounded-md border border-border/20 font-mono text-sm text-foreground/90"
                        >
                  ...
                  {parsedOutput.pending_mcp_servers && parsedOutput.pending_mcp_servers.length > 0 && (
                    <div className="text-xs text-muted-foreground/70 p-2 bg-muted/10 rounded">
                ...
                <div className="space-y-3">
                  ...
                  {parsedOutput.attachments && parsedOutput.attachments.length > 0 && (
                    <div className="space-y-1.5">
                      <div className="text-xs text-muted-foreground/70 font-medium">{t('toolExecution.attachments')}:</div>
                      {parsedOutput.attachments.map((att: any, idx: number) => (
                        <div
                          key={idx}
                          className="flex items-center gap-2 p-2 bg-muted/20 rounded-md border border-border/20 text-xs"
                        >
              ...
              {toolName === 'StructuredOutput' && 'structured_output' in parsedOutput && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-foreground/90">{t('toolExecution.structuredOutput')}</span>
                  </div>

                  <div className="space-y-2">
                    {Object.entries(parsedOutput.structured_output).map(([key, value]) => (
                      <div key={key} className="p-3 bg-muted/20 rounded-lg border border-border/30">
                        <div className="text-xs text-muted-foreground/70 mb-1 font-medium">{key}</div>
              ...
              {!['Sleep', 'Config', 'ToolSearch', 'SendUserMessage', 'StructuredOutput'].includes(toolName) && (
                <pre className="text-xs text-foreground/80 font-mono whitespace-pre-wrap bg-muted/20 px-3 py-2 rounded overflow-auto max-h-64">
