...
export function DelegationBlock({
  ...
  return (
    <div className="my-1.5 bg-indigo-500/5 dark:bg-indigo-500/10 rounded-lg border border-indigo-500/20 overflow-hidden">
      {/* Header */}
      <div className="flex items-center gap-2 px-3 py-2 bg-indigo-500/10 border-b border-indigo-500/20">
        ...
        />
        <Icon className="h-3.5 w-3.5 shrink-0 text-indigo-500/70" />
        <span className={cn('text-xs font-medium text-foreground', isError && 'text-red-400')}>{label}</span>
        <span className="text-muted-foreground/40">|</span>
        <span className="text-xs font-medium flex-1 text-foreground/80 truncate">{name}</span>
        
        {toolName === 'Agent' && inputParams.subagent_type && (
          <>
            <span className="text-muted-foreground/40">|</span>
            <span className="text-[10px] text-muted-foreground/70">{t('delegation.subagentType')}: {inputParams.subagent_type}</span>
          </>
        )}
        {toolName === 'Agent' && inputParams.model && (
          <>
            <span className="text-muted-foreground/40">|</span>
            <span className="text-[10px] text-muted-foreground/70">{inputParams.model}</span>
          </>
        )}
        
        {isPending && <span className="text-[10px] text-indigo-500 animate-pulse">{t('delegation.processing')}</span>}
        {isCancelled && <span className="text-red-400 text-[10px] font-medium bg-red-400/10 px-1.5 py-0.5 rounded-md">{t('delegation.stopped')}</span>}
        {isError && !isCancelled && <span className="text-red-400 text-[10px] font-medium bg-red-400/10 px-1.5 py-0.5 rounded-md">{t('delegation.error')}</span>}
      ...
      {hasDetails && (
        <>
          <button
            ...
            className="w-full flex items-center gap-1.5 px-3 py-1.5 bg-indigo-500/5 hover:bg-indigo-500/10 transition-colors text-left border-b border-indigo-500/10"
          >
            {isExpanded ? (
              <ChevronDown className="h-3.5 w-3.5 text-indigo-500/70" />
            ) : (
          ...
          {isExpanded && (
            <div className="px-3 py-2 bg-indigo-500/5 space-y-2">
              {toolName === 'Skill' && parsedOutput && 'path' in parsedOutput && (
                <>
                  <div className="flex items-start gap-1.5 text-[10px]">
                    <FileText className="h-3 w-3 mt-0.5 text-indigo-500/70 shrink-0" />
                  ...
                  {parsedOutput.description && (
                    <div className="text-xs text-foreground/80">
                      <div className="text-muted-foreground/70 text-[10px] mb-1">{t('delegation.description')}:</div>
                      {parsedOutput.description}
                    </div>
                  )}
